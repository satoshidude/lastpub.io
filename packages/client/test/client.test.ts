...
describe('LastpubClient E2E (Mini-Relay + Tower)', () => {
  ...
  it('Create → check-in (with edit) → delete, capsule stays consistent', async () => {
    ...
    expect(running.db.getJobByRequestId(firstMsg.placement.requestId)?.status).toBe('scheduled')
    // job is keyed to the message's own slot (§3.2)
    expect(running.db.getJobByRequestId(firstMsg.placement.requestId)?.slot).toBe(firstMsg.id)
    expect(store.switch?.messages[0]?.placement?.requestId).toBe(firstMsg.placement.requestId)
    ...
    const before = firstMsg.placement.requestId
    ...
    expect(afterMsg.placement.requestId).not.toBe(before)
    expect(running.db.getJobByRequestId(before)).toBeUndefined()
    expect(running.db.getJobByRequestId(afterMsg.placement.requestId)?.status).toBe('scheduled')
    ...
    // export contains the current wrap and its single job placement
    ...
    expect(exp.job.request_id).toBe(afterMsg.placement.requestId)
    expect(exp.job.tower).toBe(running.towerPub)

    // delete (§4.4): silent, job hard gone
    await client.deleteSwitch(after)
    expect(running.db.getJobByRequestId(afterMsg.placement.requestId)).toBeUndefined()
  ...
  it('restore from export and from relay reconstructs a resumable switch', async () => {
    ...
    expect(rx.messages[0].placement.requestId).toBe(msg.placement.requestId)
    ...
    expect(rr?.messages[0].placement.requestId).toBe(msg.placement.requestId)
    expect(rr?.messages[0].wrap.id).toBe(msg.wrap.id)
    expect((await fromRelay.readDraft(rr!)).message).toBe('recover me')
    fromRelay.close()
  }, 30_000)

  it('migration: a check-in moves the switch to the tower in settings', async () => {
    ...
    const oldReq = sw.messages[0].placement.requestId
    ...
    // install pointed at a live tower. The check-in migrates the switch: the new
    // job lands at B and the old job at A is cancelled.
    const atB = new LastpubClient(
      a,
      { relays: [relay.url], towerNpub: nip19.npubEncode(towerB.towerPub) },
      store,
    )
    const after = await atB.checkin(sw)
    expect(after.towerPub).toBe(towerB.towerPub)
    expect(after.messages[0].placement.towerPub).toBe(towerB.towerPub)
    expect(towerB.db.getJobByRequestId(after.messages[0].placement.requestId)?.status).toBe(
      'scheduled',
    )
    await new Promise((r) => setTimeout(r, 200)) // let tower A process the drop-cancel
    expect(running.db.getJobByRequestId(oldReq)).toBeUndefined() // old tower's job cancelled
